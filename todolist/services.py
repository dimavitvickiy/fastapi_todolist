...
from todolist.database.processors import create_todo_list, get_todo_lists, get_todo_list
...
class TodoListService:
    ...
    async def get(self, todo_list_id):
        todo_list_doc = await get_todo_list(todo_list_id)
        todo_list = TodoList(id=str(todo_list_doc["_id"]), **todo_list_doc)
        return todo_list

    async def update(self, todo_list):
        return todo_list

    async def delete(self, todo_list_id):
        return todo_list_id
